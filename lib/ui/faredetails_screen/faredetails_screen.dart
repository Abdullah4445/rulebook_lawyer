...
class _FareDetailsScreenState extends State<FareDetailsScreen> {
  ...
  // --- YEAH FUNCTION UPDATE KIYA GAYA HAI ---
    ...
    try {
      ...
      if (fare['type'] == 'case_total') {
        Map<String, dynamic> updates = {
          ...
          'fareDetails.lawyerStatus': 'done', // Lawyer status updated to done
          'fareDetails.driverConfirmed': true,
        };
        if (fare['steps'] != null) {
          List steps = List.from(fare['steps']);
          steps[0]['customerStatus'] = 'done';
          steps[0]['lawyerStatus'] = 'done'; // In-step lawyer status updated
        ...
        steps[stepIndex]['lawyerStatus'] = 'done'; // Lawyer status updated to done for specific step
        steps[stepIndex]['driverConfirmed'] = true;
        await docRef.update({'fareDetails.steps': steps});
      }

      await fetchFareDetails();
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Payment & Lawyer Step Confirmed')));
    } catch (e) {
      ...
      child: Column(
        ...
        children: [
          Row(
            ...
            children: [
              Text("STEP ${index + 1}", style: GoogleFonts.poppins(fontSize: 11, fontWeight: FontWeight.bold, color: Colors.blueGrey)),
          ...
          Row(
            ...
            children: [
              Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  const Text("Fee", style: TextStyle(fontSize: 12, color: Colors.grey)),
                  Text("Rs $price", style: GoogleFonts.poppins(fontSize: 17, fontWeight: FontWeight.bold, color: Colors.green[800])),
                ],
              ),
              Column(
  ...
  Widget _buildLawyerActionBtn(VoidCallback onMarkDone) {
    return SizedBox(
      ...
      child: ElevatedButton(
        ...
        style: ElevatedButton.styleFrom(backgroundColor: Colors.blue[800], foregroundColor: Colors.white, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8))),
  ...
  Widget _buildDriverConfirmationRow(int index) {
    return Column(
      children: [
        const Text("Customer claims payment is done. Confirm?", style: TextStyle(fontSize: 12, fontStyle: FontStyle.italic, color: Colors.redAccent)),
        const SizedBox(height: 8),
        Row(
          children: [
            Expanded(
  ...
  Widget _completedBanner(String text) {
    return Container(
      ...
      child: Text(text, textAlign: TextAlign.center, style: TextStyle(color: Colors.green[800], fontWeight: FontWeight.bold, fontSize: 13)),
    );
  }
  ...
  Widget _statusBadge(String status, String prefix) {
    ...
    return Container(
      ...
      child: Text("$prefix: ${status.toUpperCase()}", style: TextStyle(fontSize: 9, color: color, fontWeight: FontWeight.bold)),
    );
  }
  ...
  Widget _buildTotalSection() {
    return Container(
      ...
      child: Row(
        mainAxisAlignment: MainAxisAlignment.spaceBetween,
        children: [
          Text("Grand Total", style: GoogleFonts.poppins(fontSize: 16, fontWeight: FontWeight.w600)),
          Text("Rs ${fareDetails['total'] ?? 0}", style: GoogleFonts.poppins(fontSize: 20, fontWeight: FontWeight.bold, color: Colors.blue[900])),
        ],
      ),
    );
  }
